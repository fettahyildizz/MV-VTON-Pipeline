...
class LeakyReLUBatchNorm2d(nn.BatchNorm2d):
    def __init__(self, num_features, negative_slope=0.01, **kwargs):
        super(LeakyReLUBatchNorm2d, self).__init__(num_features, **kwargs)
        self.negative_slope = negative_slope

    def forward(self, input):
        # Apply BatchNorm2d as usual
        normalized = super().forward(input)
        # Apply LeakyReLU
        return F.leaky_relu(normalized, negative_slope=self.negative_slope)

def InPlaceABNSync(num_features):
    return LeakyReLUBatchNorm2d(num_features)

...
class FrozenCLIPImageEmbedder(AbstractEncoder):
    ...
    def __init__(self, version="openai/clip-vit-large-patch14", local_files_only: bool = False, cache_dir: str = None):
        super().__init__()
        # `version` can be either:
        # - a HuggingFace model id: "openai/clip-vit-large-patch14"
        # - a local directory containing config.json + model weights
        # The original repo had a hard-coded local path; make it portable.
        import os
        version = os.environ.get("MVVTON_CLIP_VISION", version)
        if isinstance(version, str) and (version.startswith("/") or version.startswith("./") or version.startswith("../")):
            if not os.path.exists(version):
                raise FileNotFoundError(
                    f"CLIP vision model path does not exist: {version}. "
                    "Set MVVTON_CLIP_VISION to a valid local path, or use a HuggingFace model id like 'openai/clip-vit-large-patch14'."
                )
